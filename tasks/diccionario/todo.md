...
- [ ] **T1.1** `lib/dictionary/types.ts`, `registry.ts` (imports estáticos), `getDictionary()`, semilla `data/dictionary/ashaninka.json` con **3 entradas** (una con ejemplo) `[AC-M1-9 parcial]`
...
- [ ] **T2.1** `lib/dictionary/text.ts`: `normalize` (preserva `ñ`), `slugify`, `compareWords`. **Tests primero, en rojo** `[AC-M1-1, AC-M1-2]`
- [ ] **T2.2** `resolveWord()`: por `id`, luego por `word`/variante normalizada. Tests primero `[AC-M1-8]`
